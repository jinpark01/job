...
get_navigator = {
	...
	'action' : function(){
    	...
    	var d3_txt = '<?=$_tit[2]?>';
    	var $d3 = '';
    	$('#navigator li.d3').append('<ul>'+$('#gnb > li.active > ul > li.active').html()+'</ul>');
    	$('#navigator li.d3 > a').attr('href',$('#gnb > li.active > ul > li.active > ul > li.active > a').attr('href'));
...
$('.visual .roll').on('touchstart', e => {
  $('.visual .roll').slick('slickPlay');
});
	...
	'action' : function(){
        ...
        $ele.win.on('load scroll', function(){
    		$('#main .section').each(function(i){
                ...
                if(getdevice()=="pc"){
                    if( ( $ele.win.scrollTop() + ($ele.win.height() / 2) ) > $('._sec').eq(i).offset().top ){
        				$(this).addClass('active');
        			}else{
        				$(this).removeClass('active');
        			}
                } else if(getdevice()=="mo"){
                    if( ( $ele.win.scrollTop() + $ele.win.height() / 10) > $('._sec').eq(i).offset().top ){
        				$(this).addClass('active');
        			}else{
        				$(this).removeClass('active');
        			}
